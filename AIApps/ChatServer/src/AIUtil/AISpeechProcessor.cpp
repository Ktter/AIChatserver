...
std::string AISpeechProcessor::recognize(const std::string &speechData,
                                         const std::string &format, int rate,
                                         int channel) {
  CURL *curl = curl_easy_init();
  if (!curl)
    return "";
  ...
  try {
    ...
    if (root.contains("result") && root["result"].is_array() &&
        !root["result"].empty()) {
...
std::string AISpeechProcessor::synthesize(const std::string &text,
                                          ...
                                          const std::string &lang, int speed,
  ...
  std::string create_url =
      "https://aip.baidubce.com/rpc/2.0/tts/v1/create?access_token=" + token_;
  ...
  json body = {{"text", text},        {"format", format}, {"lang", lang},
  ...
  try {
    ...
    // 有些示例中 task_id 在根节点或者在 tasks_info[0] 中都可能出现，优先取根
    // task_id，再尝试 tasks_info
    if (result_json.contains("task_id") && result_json["task_id"].is_string()) {
      task_id = result_json["task_id"].get<std::string>();
    } else if (result_json.contains("tasks_info") &&
               result_json["tasks_info"].is_array() &&
               !result_json["tasks_info"].empty() &&
               result_json["tasks_info"][0].contains("task_id")) {
  ...
  while (loops++ < max_loops) {
    ...
    std::string query_url =
        "https://aip.baidubce.com/rpc/2.0/tts/v1/query?access_token=" + token_;
    ...
    // 解析轮询结果
    // 请求成功就返回speech_url 音频下载地址
    try {
      json queryResult = json::parse(response);
      if (queryResult.contains("tasks_info") &&
          queryResult["tasks_info"].is_array() &&
